...
bool ReferenceLine::ComputePathProfile(
  ...
  //获取 y 和 x 各自对 s 的有限差分近似一阶导数以进行曲率计算
  for (std::size_t i = 0; i < points_size; ++i) {
    double xds = 0.0;
    double yds = 0.0;
    if (i == 0) {
  ...
  //获取 y 和 x 各自对 s 的有限差分逼近二阶导数以进行曲率计算
  for (std::size_t i = 0; i < points_size; ++i) {
    double xdds = 0.0;
    double ydds = 0.0;
    if (i == 0) {
